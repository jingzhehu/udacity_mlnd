...
class Environment(object):
    ...
    def reset(self):
        ...
        # deadline definition
        deadline = self.compute_dist(start, destination) * 4
    ...
    def step(self, trial=0):
        ...
        # Update agents
        for agent in self.agent_states.iterkeys():
            agent.update(self.t, trial=trial)
    ...
    def sense(self, agent):
        ...
        state = self.agent_states[agent] #state of current agent
    ...
    def act(self, agent, action):
        ...
        if agent is self.primary_agent:
            a = 1 # for setting debugging purposes, setting break point
        ...
        if move_okay:
            ...
            if action is not None:
                # Valid non-null move
                location_i = (location[0] + heading[0] - self.bounds[0]) % \
                             (self.bounds[2] - self.bounds[0] + 1) + self.bounds[0]

                location_j = (location[1] + heading[1] - self.bounds[1]) % \
                             (self.bounds[3] - self.bounds[1] + 1) + self.bounds[1]

                location = (location_i, location_j)

                # wrap-around
                # if self.bounds[0] <= location[0] <= self.bounds[2] and
                # self.bounds[1] <= location[1] <= self.bounds[3]:  # bounded
